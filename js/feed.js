// Логика ленты с видео, геолокацией и упоминаниями
...
function parseMentions(text) {
    ...
    while ((match = mentionRegex.exec(text)) !== null) {
        ...
        if (user) {
            html = html.replace(`@${username}`, `<span class="mention" style="color: #0095f6; cursor: pointer;" onclick="goToProfile('${username}')">@${username}</span>`);
        }
    }
    return html;
}
...
function getCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
            ...
            currentLocation = data.display_name?.split(',')[0] || `${latitude.toFixed(4)}, ${longitude.toFixed(4)}`;
...
function createPost(content, imageData, videoData = null, location = null) {
    const currentUser = getCurrentUser();
    if (!currentUser) return;
    const posts = getPosts();
    const newPost = {
...
function toggleSave(postId) {
    toggleSavePost(getCurrentUser().id, postId);
    loadFeed();
}
    ...
    if (post) {
        ...
        if (post.userId !== currentUser.id) {
            addNotification(post.userId, {
                type: 'comment',
                fromUser: currentUser.username,
                postId: currentPostForComment,
                message: `${currentUser.username} оставил комментарий: "${commentText.substring(0, 50)}..."`
            });
        }
    }
}
...
function createPostFromModal() {
    ...
    if (!content.trim() && !imageInput.files[0] && !videoInput.files[0]) {
        alert('Напишите подпись или добавьте фото/видео');
        return;
    }
    
    const processUpload = (file, isVideo) => {
        const reader = new FileReader();
        reader.onload = function(e) {
            if (isVideo) {
                createPost(content, null, e.target.result, location);
            } else {
                createPost(content, e.target.result, null, location);
            }
    ...
    if (videoInput.files[0]) {
        processUpload(videoInput.files[0], true);
    } else if (imageInput.files[0]) {
        processUpload(imageInput.files[0], false);
    } else {
        createPost(content, null, null, location);
        closeCreatePostModal();
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (user && window.location.pathname.includes('feed.html')) {
        ...
        const updatePreview = (file, isVideo) => {
            const reader = new FileReader();
            reader.onload = function(e) {
                if (isVideo) {
        ...
        if (imageInput) {
            imageInput.addEventListener('change', function() {
                if (this.files && this.files[0]) updatePreview(this.files[0], false);
            });
        }
        if (videoInput) {
            videoInput.addEventListener('change', function() {
                if (this.files && this.files[0]) updatePreview(this.files[0], true);
            });
        }
    }
});
